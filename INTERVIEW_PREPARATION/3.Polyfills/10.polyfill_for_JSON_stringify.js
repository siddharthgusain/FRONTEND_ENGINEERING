...
// We have to handle primitive types like Number, string , boolean
// We have to handle nested objects and array
// Then some base cases 
...
const JSONStringifyPolyfill = (obj) => {
    ...
    if (isArray(obj)) {
        ...
        obj.forEach((eachValue) => {
            arrStr += arrayValuesNullTypes(eachValue) ? JSONStringifyPolyfill(null) : JSONStringifyPolyfill(eachValue);
            arrStr += ','
        });
    ...
    if (isObject(obj)) {
        ...
        objKeys.forEach((eachKey) => {
            ...
            objStr += (!ignoreDataTypes(eachValue)) ? `"${eachKey}":${JSONStringifyPolyfill(eachValue)},` : '';
        });
        return `{` + removeComma(objStr) + `}`;
    }
};

console.log(JSONStringifyPolyfill(obj))